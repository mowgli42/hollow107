...
import { ROLE_LABEL, defaultRouteForRole, type Role } from "@/lib/hollow107";
...
export function RoleSwitch() {
  ...
  function pick(next: Role) {
    setRole(next);
    void navigate({ to: defaultRouteForRole(next) });
  }

  return (
    <div role="radiogroup" aria-label="Your role" className="inline-flex rounded-md border border-border-strong bg-bg-elevated p-0.5">
      {ROLES.map((r) => {
        const active = role === r;
        return (
          <button
